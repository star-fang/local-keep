class PhysicsEngine {
    ...
    activateZoomModule(size, actingPoint) {
        ...
        function zoom( pos, scale, onMove, factor, delta, zoomPos) {
            ...
            const min_scale = landscapeMode ? bgHeight / size.height / 2 : bgWidth / size.width / 2;
    ...
    smoothMove(left, top, onComplete) {
        const initLeft = this.pos.left;
        const leftTotalProgress = left - initLeft;
        const initTop = this.pos.top;
        const topTotalProgress = top - initTop;
        ...
        this.target.animate({
            ...
            {
                step: function( currLeft, fx ) {
                    const progress = (currLeft - initLeft) / leftTotalProgress; // 0 ~ 1
                    const currTop = initTop + progress * topTotalProgress;
